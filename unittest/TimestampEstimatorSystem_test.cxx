...
BOOST_AUTO_TEST_CASE(Basics)
{
  ...
  dunedaq::timinglibs::TimestampEstimatorSystem tes(clock_frequency_hz);

  BOOST_CHECK_EQUAL(tes.wait_for_valid_timestamp(continue_flag), dunedaq::timinglibs::TimestampEstimatorBase::kFinished);

  std::atomic<bool> do_not_continue_flag{false};
  BOOST_CHECK_EQUAL(tes.wait_for_valid_timestamp(do_not_continue_flag), dunedaq::timinglibs::TimestampEstimatorBase::kInterrupted);

  dunedaq::dfmessages::timestamp_t ts_now=tes.get_timestamp_estimate();
  BOOST_CHECK_EQUAL(tes.wait_for_timestamp(ts_now+clock_frequency_hz, continue_flag), dunedaq::timinglibs::TimestampEstimatorBase::kFinished);

  ts_now=tes.get_timestamp_estimate();
  BOOST_CHECK_EQUAL(tes.wait_for_timestamp(ts_now+clock_frequency_hz, do_not_continue_flag), dunedaq::timinglibs::TimestampEstimatorBase::kInterrupted);
