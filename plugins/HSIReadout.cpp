    ...
    try
    {
      ...
      if (hsi_words.size() >= 5) {
        ...
        // bits 15-0 contain the sequence counter
          ...
          TLOG_DEBUG(3) << get_name() << ": read out data: " << header << ", " << std::hex << ts << ", " << data << ", " << std::bitset<32>(trigger) << ", " << "\n";
...
void
HSIReadout::update_buffer_counts(uint16_t new_count) {
  std::unique_lock mon_data_lock(m_buffer_counts_mutex);
  if (m_buffer_counts.size() > 1000) m_buffer_counts.pop_front();
...
HSIReadout::read_average_buffer_counts() {
  ...
  double total_counts;
  uint32_t number_of_counts;

  total_counts = 0;
  number_of_counts = m_buffer_counts.size();

  if (number_of_counts) {
   for (uint i=0; i < number_of_counts; ++i) {
